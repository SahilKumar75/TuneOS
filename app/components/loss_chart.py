...
def loss_chart() -> rx.Component:
    """Live training chart: train loss, eval loss, learning rate, + optional grad norm."""
    # ── shared building blocks ────────────────────────────────────────────────
    _train = rx.recharts.line(
        data_key="loss",
        stroke="#3b82f6",
        stroke_width=2,
        dot=False,
    ...
    _data_prop = dict(data=TrainingPollerState.loss_history_chart_data, width="100%", height=280)

    return rx.vstack(
        rx.cond(
            TrainingPollerState.show_grad_norm,
            rx.recharts.composed_chart(_train, _eval, _lr, _grad, *_axes, **_data_prop),
            rx.recharts.composed_chart(_train, _eval, _lr, *_axes, **_data_prop),
        ),
        rx.hstack(
            rx.switch(
                checked=TrainingPollerState.show_grad_norm,
                on_change=TrainingPollerState.set_show_grad_norm,
                size="1",
            ),
            rx.text("Show gradient norm", font_size="0.76rem", color=c("text_muted")),
            spacing="2",
            align="center",
        ),
        spacing="2",
