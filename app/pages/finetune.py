    ...
    return rx.vstack(
        rx.box(
            rx.cond(
                ...
                rx.text(str(step_num), font_size="0.72rem", font_weight="600",
                        color=rx.cond(is_active, "white", c("text_muted"))),
            ),
            width="26px", height="26px", border_radius="50%",
            background=rx.cond(is_done, c("success"), rx.cond(is_active, c("accent"), c("bg_input"))),
            border="2px solid",
            border_color=rx.cond(is_active | is_done, c("accent"), c("border")),
            display="flex", align_items="center", justify_content="center",
        ),
        rx.text(_STEP_LABELS[index], font_size="0.68rem",
        ...
        *[
            rx.hstack(
                ...
                rx.box(height="2px", flex="1",
                       background=rx.cond(FinetuneState.current_step > i + 1, c("accent"), c("border")),
                       min_width="20px")
                if i < len(_STEP_LABELS) - 1 else rx.fragment(),
                spacing="0", align="center",
                flex="1" if i < len(_STEP_LABELS) - 1 else "0",
            )
            for i in range(len(_STEP_LABELS))
        ],
        width="100%", max_width="680px", align="center", justify="center", margin_bottom="32px",
    )
    ...
    return _card(
        rx.vstack(
            rx.text(label, font_size="0.68rem", font_weight="500",
                    color=c("text_muted"), letter_spacing="0.05em"),
            rx.text(value, font_size="1.5rem", font_weight="700",
                    color=c("text_primary")),
            spacing="1",
        ),
        padding="14px",
...
def _epoch_log_row(entry: dict) -> rx.Component:
    return rx.hstack(
        rx.text(f"Epoch {entry['epoch']}", font_size="0.78rem", font_weight="500",
                color=c("text_primary"), width="60px"),
        rx.text(
            f"loss {entry['loss_start']} → {entry['loss_end']}",
            font_size="0.78rem", color=c("text_secondary"), flex="1",
        ),
        rx.text(
...
def _step5() -> rx.Component:
    return rx.vstack(
        ...
        rx.hstack(
            rx.vstack(
                rx.text("Training", font_size="1.1rem", font_weight="700",
                        color=c("text_primary")),
                rx.text(FinetuneState.effective_model_name, font_size="0.82rem",
                        color=c("text_muted")),
                spacing="0",
            ),
            rx.spacer(),
        ...
        rx.vstack(
            rx.hstack(
                ...
                rx.spacer(),
                rx.text(FinetuneState.epoch_progress_pct.to_string() + "%",
                        font_size="0.76rem", color=c("text_secondary")),
            ),
            rx.progress(value=FinetuneState.epoch_progress_pct, max=100,
        ...
        rx.cond(
            ...
            _card(
                rx.hstack(
                    rx.icon("sparkles", size=16, color=c("accent")),
                    rx.text(FinetuneState.ai_commentary, font_size="0.86rem",
                            color=c("text_primary")),
                    spacing="2", align="flex-start",
                )
            ),
            rx.fragment(),
        ),
        ...
        rx.cond(
            ...
            _card(
                rx.vstack(
                    rx.text("Epoch log", font_size="0.78rem", font_weight="600",
                            color=c("text_secondary"), margin_bottom="8px"),
        ...
        rx.cond(
            ...
            rx.vstack(
                rx.callout(
                    rx.hstack(rx.icon("check-circle", size=16),
                              rx.text("Training complete! Advancing to results..."),
                              spacing="2"),
                    color_scheme="green",
                ),
                rx.button("View Results →", on_click=FinetuneState.go_to_step(6),
...
def _step6() -> rx.Component:
    return rx.vstack(
        ...
        _card(
            rx.vstack(
                rx.hstack(
                    rx.text("Evaluation metrics", font_size="0.9rem", font_weight="600",
                            color=c("text_primary")),
                    rx.spacer(),
                    rx.cond(
                        FinetuneState.eval_status == "idle",
                        rx.button("Run evaluation", on_click=FinetuneState.run_eval,
                ...
                rx.cond(
                    ...
                    rx.grid(
                        rx.vstack(
                            ...
                            rx.text(FinetuneState.eval_perplexity.to_string(),
                                    font_size="1.8rem", font_weight="700", color=c("accent")),
                        ...
                        rx.vstack(
                            ...
                            rx.text(
                                ...
                                font_size="0.84rem", color=c("text_secondary"),
                            ),
                            spacing="1",
                        ),
                        columns="2", spacing="4",
        ...
        _card(
            rx.vstack(
                rx.text("Test your model", font_size="0.9rem", font_weight="600",
                        color=c("text_primary"), margin_bottom="8px"),
                rx.cond(
                    FinetuneState.user_intent != "",
                    rx.text(f"System context: {FinetuneState.user_intent}",
                            font_size="0.76rem", color=c("text_muted"), font_style="italic"),
                    rx.fragment(),
                ),
                # Chat history
                rx.cond(
                        ...
                        rx.foreach(
                            ...
                            lambda msg: rx.box(
                                rx.text(
                                    ...
                                    border_radius="8px",
                                    align_self=rx.cond(msg["role"] == "user",
                                                       "flex-end", "flex-start"),
                                    max_width="80%",
                                ),
                                display="flex",
                                flex_direction=rx.cond(msg["role"] == "user",
                                                       "row-reverse", "row"),
                                width="100%",
                                margin_bottom="6px",
                            )
                        ),
                        width="100%", max_height="300px", overflow_y="auto",
                        padding="8px", border="1px solid", border_color=c("border"),
                        border_radius="8px",
                    ),
                    rx.fragment(),
                ),
                rx.hstack(
                    ...
                    rx.button(
                        rx.cond(FinetuneState.chat_loading, rx.spinner(size="2"),
        ...
        rx.cond(
            ...
            _card(
                rx.vstack(
                    rx.text("Past runs with this setup", font_size="0.9rem", font_weight="600",
                            color=c("text_primary"), margin_bottom="8px"),
                    rx.table.root(
                        rx.table.header(
                            rx.table.row(
                        ...
                        rx.table.body(
                            rx.foreach(
                                ...
                                lambda r: rx.table.row(
                                    ...
                                    rx.table.cell(rx.text(r.model_id.split("/")[-1],
                                                         font_size="0.8rem")),
                                    rx.table.cell(rx.text(r.epochs.to_string(),
                                                         font_size="0.8rem")),
                                    rx.table.cell(rx.text(r.final_loss.to_string(),
                                                         font_size="0.8rem")),
                                    rx.table.cell(rx.text(r.perplexity.to_string(),
                                                         font_size="0.8rem")),
                                )
                            )
                        ),
...
def _deploy_target_row(
    target_key: str, label: str, description: str, icon: str, is_checked,
) -> rx.Component:
    return rx.hstack(
        rx.checkbox(
            checked=is_checked,
        ...
        spacing="3", align="flex-start", padding="10px 0",
        border_bottom="1px solid", border_color=c("border"),
        width="100%",
    )
...
def _step7() -> rx.Component:
    return rx.vstack(
        ...
        rx.text("Choose how you want to export or share your fine-tuned adapter.",
                font_size="0.86rem", color=c("text_secondary"), margin_bottom="16px"),
        ...
        rx.cond(
            ...
            _card(
                rx.vstack(
                    rx.text("Hugging Face Hub", font_weight="600", font_size="0.88rem",
                            color=c("text_primary")),
                    rx.grid(
                        rx.vstack(
                            _label("HF Token"),
                            rx.input(type="password", placeholder="hf_xxxxxxxxxxxxx",
                                     value=FinetuneState.hf_token_input,
                                     on_change=FinetuneState.set_hf_token_input, width="100%"),
                            spacing="1",
                        ),
                        rx.vstack(
                            _label("Repository name (e.g. myuser/my-chatbot-lora)"),
                            rx.input(placeholder="username/repo-name",
                                     value=FinetuneState.hf_repo_name,
                                     on_change=FinetuneState.set_hf_repo_name, width="100%"),
                            spacing="1",
                        ),
                        columns="2", spacing="3", width="100%",
                    ),
                    rx.button(
                        rx.cond(FinetuneState.push_status == "pushing",
        ...
        rx.cond(
            ...
            _card(
                rx.vstack(
                    rx.text("GGUF Export", font_weight="600", font_size="0.88rem",
                            color=c("text_primary")),
                    rx.callout(
                    ...
                    rx.hstack(
                        ...
                        rx.button(
                            rx.cond(FinetuneState.gguf_status == "exporting",
                                    rx.hstack(rx.spinner(size="2"), rx.text("Exporting..."),
                                              spacing="2"),
                                    rx.text("Export GGUF")),
                            on_click=FinetuneState.start_gguf_export,
                            disabled=FinetuneState.gguf_status == "exporting",
                            color_scheme="blue", size="2", align_self="flex-end",
                        ),
                        spacing="3", align="flex-end",
                    ),
                    spacing="3",
                )
            ),
        ...
        rx.cond(
            ...
            _card(
                rx.vstack(
                    rx.text("GitHub Push", font_weight="600", font_size="0.88rem",
                            color=c("text_primary")),
                    rx.grid(
                        rx.vstack(
                            _label("Repository URL (HTTPS)"),
                            rx.input(placeholder="https://github.com/user/repo",
                                     value=FinetuneState.github_repo_url,
                                     on_change=FinetuneState.set_github_repo_url, width="100%"),
                            spacing="1",
                        ),
                        rx.vstack(
                            _label("GitHub Token (needs repo scope)"),
                            rx.input(type="password", placeholder="ghp_xxxxxxxxxxxxx",
                                     value=FinetuneState.github_token,
                                     on_change=FinetuneState.set_github_token, width="100%"),
                            spacing="1",
                        ),
                        columns="2", spacing="3", width="100%",
                    ),
                    rx.button(
                        rx.cond(FinetuneState.github_push_status == "pushing",
                                rx.hstack(rx.spinner(size="2"), rx.text("Pushing..."), spacing="2"),
                    ...
                    rx.cond(
                        ...
                        rx.callout(f"Pushed to {FinetuneState.github_repo_url}",
                                   color_scheme="green", size="1"),
                        rx.fragment(),
                    ),
                    spacing="3",
        ...
        rx.hstack(
            ...
            rx.cond(
                ...
                rx.button(
                    rx.cond(FinetuneState.merge_status == "merging",
                            rx.hstack(rx.spinner(size="2"), rx.text("Merging..."), spacing="2"),
                            rx.hstack(rx.icon("layers", size=14),
                                      rx.text("Merge & download"), spacing="2")),
        ...
        rx.cond(
            ...
            _card(
                rx.vstack(
                    rx.text("Activity log", font_size="0.78rem", font_weight="600",
                            color=c("text_secondary")),
                    rx.box(
                        rx.text(FinetuneState.deploy_log,
                                font_size="0.76rem", color=c("text_secondary"),
                                font_family="monospace", white_space="pre-wrap"),
                        background=c("bg_input"), border_radius="8px", padding="12px",
        ...
        # Start over
        rx.box(height="8px"),
        rx.button("Start a new fine-tune →",
    ...
    return rx.box(
        rx.vstack(
            ...
            rx.hstack(
                ...
                rx.text("Fine-tune a model", font_size="1.25rem", font_weight="700",
                        color=c("text_primary")),
                spacing="2", align="center", margin_bottom="24px",
            ),
